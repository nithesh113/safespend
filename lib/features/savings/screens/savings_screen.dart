...
class _SavingsScreenState extends State<SavingsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (mounted) _err(e.userMessage);
    } catch (_) {
      if (mounted) _err('Failed to load savings goals.');
    } finally {
  ...
  Future<void> _refreshDashboard() async {
    try {
      if (mounted) await context.read<DashboardProvider>().loadDashboardData();
    } catch (_) {}
  }
  ...
  Future<void> _createGoal() async {
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(
                  ...
                  keyboardType: TextInputType.number,
                  decoration: const InputDecoration(
                    labelText: 'Planned monthly saving (¥)',
                    filled: true,
                  ),
                ),
                const SizedBox(height: 12),
                ListTile(
                  ...
                  title: Text(
                    target == null
                        ? 'Optional deadline'
                        : DateFormat('MMM dd, yyyy').format(target!),
                  ),
                  trailing: target != null
                      ? IconButton(
                          icon: const Icon(Icons.clear),
                          onPressed: () => sd(() => target = null),
                        )
                      : null,
                  onTap: () async {
    ...
    final targetAmount = double.tryParse(amtCtrl.text.trim()) ?? 0;
    final monthlyContribution = double.tryParse(monthlyCtrl.text.trim()) ?? 0;
    if (targetAmount <= 0) {
      _err('Enter a target amount greater than zero.');
      return;
    }
    try {
      ...
      sm.showSnackBar(
        SnackBar(
          content: const Text('Failed to create goal.'),
          backgroundColor: Theme.of(context).colorScheme.error,
          behavior: SnackBarBehavior.floating,
        ),
      );
                ...
                onPressed: () async {
                  ...
                  try {
                    ...
                    if (ctx.mounted) {
                      ...
                      sm.showSnackBar(
                        SnackBar(
                          content: Text('Added ${formatCurrency(a)}'),
                          behavior: SnackBarBehavior.floating,
                        ),
                      );
                    }
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      return ProgressJarCard(
                        ...
                        onTap: () =>
                            _addFunds(p, g.id!, g.title, g.currentAmount),
                      );
                    },
                  ),
                ),
